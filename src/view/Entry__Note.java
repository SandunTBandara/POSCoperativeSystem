...
import view.EntryNotes.Pricechange;
import view.EntryNotes.PurchaseOrder;
import view.EntryNotes.PurchaseReturn;
import view.EntryNotes.StockExchangeIN;
import view.EntryNotes.StockExchangeOUT;
import view.EntryNotes.SupplierInvoice;
import view.EntryNotes.disposal;
...
public class Entry__Note extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        setPreferredSize(new java.awt.Dimension(1000, 941));
        ...
        kGradientPanel1.add(jPanel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(70, 60, 400, 70));
        ...
        kGradientPanel1.add(jPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(1040, 170, 400, 70));
        ...
        kGradientPanel1.add(jPanel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(1040, 60, 400, 70));
        ...
        kGradientPanel1.add(jPanel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(570, 60, 400, 70));
        ...
        kGradientPanel1.add(jPanel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(210, 300, 480, 70));
        ...
        kGradientPanel1.add(jPanel6, new org.netbeans.lib.awtextra.AbsoluteConstraints(570, 170, 400, 70));
        ...
        kGradientPanel1.add(jPanel9, new org.netbeans.lib.awtextra.AbsoluteConstraints(830, 300, 490, 70));

        jPanel11.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        btnloandetails1.setFont(new java.awt.Font("Nirmala UI", 0, 18)); // NOI18N
        btnloandetails1.setText("භාණ්ඩ ඇනවුම් සනාථ කිරීමේ  සටහන");
        ...
        kGradientPanel1.add(jPanel11, new org.netbeans.lib.awtextra.AbsoluteConstraints(70, 170, 400, 70));

        jPanel12.setBackground(new java.awt.Color(0, 0, 0));
        kGradientPanel1.add(jPanel12, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 460, 1520, 3));
        ...
        layout.setHorizontalGroup(
            ...
            .addComponent(kGradientPanel1, javax.swing.GroupLayout.DEFAULT_SIZE, 1545, Short.MAX_VALUE)
        );
    ...
    private void btndepartmentdetailsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btndepartmentdetailsActionPerformed
        try {    
            ...
            System.out.println(frontlogin.uid);
            ResultSet rs = user.getUsersPrivilage("en_order", frontlogin.uid);

            while(rs.next()){
                String type = rs.getString("Type");
                int status = rs.getInt("en_order");

                if(status == 1){
                    PurchaseOrder db = new PurchaseOrder(null, true);
    ...
    private void btnrackdetailsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnrackdetailsActionPerformed
        try {    
            ...
            ManageUser user = new ManageUser();
            ResultSet rs = user.getUsersPrivilage("en_price", frontlogin.uid);

            while(rs.next()){
                String type = rs.getString("Type");
                int status = rs.getInt("en_price");

                if(status == 1){
                    Pricechange db = new Pricechange(null, true);
                    db.setLocationRelativeTo(this);
                    db.setVisible(true);
                }else{
                    JOptionPane.showMessageDialog(null, type +" don't have permission to access this!", "SuperMarket Management", JOptionPane.ERROR_MESSAGE);
    ...
    private void btnitemdetails1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnitemdetails1ActionPerformed
        try {    
            ...
            ManageUser user = new ManageUser();
            ResultSet rs = user.getUsersPrivilage("en_orderReturn", frontlogin.uid);

            while(rs.next()){
                String type = rs.getString("Type");
                int status = rs.getInt("en_orderReturn");

                if(status == 1){
                    PurchaseReturn db = new PurchaseReturn(null, true);
                    db.setLocationRelativeTo(this);
                    db.setVisible(true);
                }else{
                    JOptionPane.showMessageDialog(null, type +" don't have permission to access this!", "SuperMarket Management", JOptionPane.ERROR_MESSAGE);
    ...
    private void btnsupplierdetails1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnsupplierdetails1ActionPerformed
        try {    
            ...
            ManageUser user = new ManageUser();
            ResultSet rs = user.getUsersPrivilage("bi_grn", frontlogin.uid);

            while(rs.next()){
                String type = rs.getString("Type");
                int status = rs.getInt("bi_grn");

                if(status == 1){
                    GoodReceiveNote db = new GoodReceiveNote(null, true);
                    db.setLocationRelativeTo(this);
                    db.setVisible(true);
                }else{
                    JOptionPane.showMessageDialog(null, type +" don't have permission to access this!", "SuperMarket Management", JOptionPane.ERROR_MESSAGE);
    ...
    private void btnsubloc1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnsubloc1ActionPerformed
        try {    
            ...
            ManageUser user = new ManageUser();
            ResultSet rs = user.getUsersPrivilage("en_exchange", frontlogin.uid);

            while(rs.next()){
                String type = rs.getString("Type");
                int status = rs.getInt("en_exchange");

                if(status == 1){
                    StockExchangeOUT db = new StockExchangeOUT(null, true);
                    db.setLocationRelativeTo(this);
                    db.setVisible(true);
                }else{
                    JOptionPane.showMessageDialog(null, type +" don't have permission to access this!", "SuperMarket Management", JOptionPane.ERROR_MESSAGE);
    ...
    private void btncategorydetailsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btncategorydetailsActionPerformed
        try {    
            ...
            ManageUser user = new ManageUser();
            ResultSet rs = user.getUsersPrivilage("en_disposal", frontlogin.uid);

            while(rs.next()){
                String type = rs.getString("Type");
                int status = rs.getInt("en_disposal");

                if(status == 1){
                    disposal db = new disposal(null, true);
                    db.setLocationRelativeTo(this);
                    db.setVisible(true);
                }else{
                    JOptionPane.showMessageDialog(null, type +" don't have permission to access this!", "SuperMarket Management", JOptionPane.ERROR_MESSAGE);
    ...
    private void btnItemAlertBinActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnItemAlertBinActionPerformed
        try {    
            ...
            while(rs.next()){
                ...
                if(status == 1){
                    StockExchangeIN db = new StockExchangeIN(null, true);
                    db.setLocationRelativeTo(this);
                    db.setVisible(true);
                }else{
                    JOptionPane.showMessageDialog(null, type +" don't have permission to access this!", "SuperMarket Management", JOptionPane.ERROR_MESSAGE);
    ...
    private void btnloandetails1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnloandetails1ActionPerformed
        try {    
            ...
            ManageUser user = new ManageUser();
            ResultSet rs = user.getUsersPrivilage("en_supInvoice", frontlogin.uid);

            while(rs.next()){
                String type = rs.getString("Type");
                int status = rs.getInt("en_supInvoice");

                if(status == 1){
                    SupplierInvoice db = new SupplierInvoice(null, true);
                    db.setLocationRelativeTo(this);
                    db.setVisible(true);
                }else{
                    JOptionPane.showMessageDialog(null, type +" don't have permission to access this!", "SuperMarket Management", JOptionPane.ERROR_MESSAGE);
